class Gameplay {
        ...
        if (this.hasApple == false) {
            ...
            while (checkSnake) {
                ...
                for (let i = 0; i < this.snake.snakeTail.length; ++i) {
                    if (this.apple.x == this.snake.snakeTail[i].x && this.apple.y == this.snake.snakeTail[i].y) {
                        continue;
                    } else {
                        checkSnake = false
                    };
    ...
    checkAppleEaten() {
        if (this.snake.snakeTail[0].x == this.apple.x && this.snake.snakeTail[0].y == this.apple.y) {
            this.snake.snakeTail[this.snake.snakeTail.length] = { x: this.apple.x, y: this.apple.y };
            this.hasApple = false;
            this.createApple();
        }
    };
    ...
    draw() {
        ...
        this.canvasContext.fillText("Score: " + (this.snake.snakeTail.length - 1), this.canvas.width - 120, 20);
        // draw apple
